...
declare const swan: {
    /**
     * 百度小游戏为开发者提供了交叉推荐能力，支持在游戏中添加交叉推荐组件
     */
    createRecommendationButton(opts?: { style?: { left: number, top: number }, type?: string }): recommendationButton

    /**
     * 获取全局唯一的 videoRecorderManager
     */
    getVideoRecorderManager(): videoRecorderManager

    /**
     * 主动拉起发布视频界面
     */
    shareVideo(opts: { videoPath: string, query?: string, title?: string, 
        success?: Function, fail?: Function, complete?: Function })

    navigateToSmartProgram(obj: { appKey: string, path?: string, extraData?: object,
        success?: Function, fail?: Function, complete?: Function })

    createRewardedVideoAd(obj: { adUnitId: string, appSid: string }): RewardedVideoAd;

    createBannerAd(obj: { adUnitId: string, appSid: string, style: { left: number, top: number, width: number, height: number } }): BannerAd;
...
declare interface recommendationButton {
...
declare interface videoRecorderManager {
    start(opts: { duration: number, microphoneEnabled: boolean })

    /**
     * 
     * @param callback recorder.onStart(res => { console.log(res.microphoneStatus); })
     */
    ...
    /**
     * 
     * @param callback recorder.onStop(res => {console.log(res.videoPath); })
     */
    onStop(callback: (res)=>void)

    onError(callback: (res)=>void)
}
