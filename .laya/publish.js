...
const zip = require(ideModuleDir + 'gulp-zip');
...
//将res目录下面的3d、2d以及audio文件夹打为zip包
gulp.task("zipRes", ["compressImage"], function() {
	return gulp.src([ `${releaseDir}/res/**`, 
		//排除LoadUI.bin和LoadUI_atlas0.png两个文件
		`!${releaseDir}/res/LoadUI.bin`, 
		`!${releaseDir}/res/LoadUI_atlas0.png` ])
        .pipe(zip('res.zip'))
        .pipe(gulp.dest(releaseDir + '/res'))
});

//打包zip后删除掉不需要的文件夹
gulp.task("delRes", ["zipRes"], function() {
	let delList = [ `${releaseDir}/res/3d/**`, `${releaseDir}/res/2d/**`, `${releaseDir}/res/audio/**`,
		`${releaseDir}/js/**.map` ]
	del(delList, { force: true })
});

// 开放域的情况下，合并game.js和index.js，并删除game.js
gulp.task("openData", ["delRes"], function (cb) {
