...
export class BdUtil {
    ...
    */ 
    ...
    private static _calBannerPos(width?: number, node?: Laya.Sprite, posType = 1)
    {
        ...
        if(node)
        {
            let gameHgt = Laya.Browser.clientHeight
            let pos = node.localToGlobal(new Laya.Point(node.x, node.y))
            let offsetY = node.height * node.pivotY + 60 //微信政策，不能完全贴近按钮，需要离开40pt，设为60安全起见

            let ratio = (gameHgt - pos.y + offsetY) / gameHgt

            y = this.sdkInfo.screenHeight * ratio

            console.log('addBanner node', node, ratio, pos.y, offsetY, gameHgt - pos.y, gameHgt, y, this.sdkInfo.screenHeight)
        }
        else
        {
     ...
     */
    static addBanner(adunit: string, width?: number, node?: Laya.Sprite, posType: number = 1, bPreload = false, 
    ...
    static createRecommendationButton(btnId: string, x: number, y: number, type = 1)
    {
        ...
        if(BdUtil.compareVersion(info.SDKVersion, '1.5.2') != -1 && swan.createRecommendationButton != null)
        {
            ...
            {
                let gameWid = Laya.Browser.width
                let gameHgt = Laya.Browser.height
